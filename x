...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local timeToPredict = getgenv().Triggerbot.Prediction.Ground  -- Using configured prediction time
...
local function shouldTriggerBotActivate(character)
    local distance = (character.PrimaryPart.Position - LocalPlayer.Character.PrimaryPart.Position).Magnitude
    local gunSettings = getgenv().Triggerbot.GunSettings
    
    -- Check if TriggerBot is active and gun type conditions
    if getgenv().Triggerbot.Active then
        if gunSettings.Enabled then
            local isGunTypeValid = false
            -- Checking if current weapon is valid
            local tool = LocalPlayer.Character:FindFirstChildOfClass("Tool")
            if tool then
                local toolType = tool.Name:lower()
                if gunSettings.Types.Pistols[toolType] or gunSettings.Types.Shotguns[toolType] or 
                   gunSettings.Types.Automatics[toolType] or gunSettings.Types.Others[toolType] then
                    isGunTypeValid = true
                end
            end
            -- Check distance settings
            if distance <= gunSettings.Range.Close then
                if gunSettings.Enabled and isGunTypeValid then
                    return true
                end
            elseif distance <= gunSettings.Range.Mid then
                if gunSettings.Enabled and isGunTypeValid then
                    return true
                end
            elseif distance <= gunSettings.Range.Far then
                if gunSettings.Enabled and isGunTypeValid then
                    return true
                end
            end
        else
            return true  -- If gun settings are disabled, always trigger
        end
    end
    return false
end
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character and shouldTriggerBotActivate(TargetPlayer.Character) then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    if os.clock() - lastClickTime >= getgenv().Triggerbot.Cooldown then
